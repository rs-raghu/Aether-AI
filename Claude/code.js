...
async function applyStylesToFigma(suggestions) {
    try {
...
async function createColorStyles(colors) {
    ...
    for (const [name, hex] of colorEntries) {
        ...
        let paintStyle = figma.getLocalPaintStyles().find(style => style.name === `AI/${capitalize(name)}`);
        if (!paintStyle) {
...
async function createSpacingVariables(spacing) {
    // Store spacing as local variables (Figma's new way to store design tokens)
    const spacingEntries = Object.entries(spacing);
    for (const [name, value] of spacingEntries) {
        try {
            const collection = figma.variables.getLocalVariableCollections().find(c => c.name === 'AI Spacing') || figma.variables.createVariableCollection('AI Spacing');
            const existingVar = figma.variables.getLocalVariables().find(v => v.name === `spacing/${name}`);
            if (!existingVar) {
